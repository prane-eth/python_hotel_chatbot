...
def resp(msg):                      # for correct responses
	if msg in hlo:
		...
		fil_nam=random.choice(['humsafar','main_yaha','majnu','morrakka','perfect','rozana','shape_of_you','taki','unity'])
		outp("Playing music...")
...
"""
Changes to code:
move voice engine init() to top
change voice() to try:            to remove dependency on speaker
		  	voice() 
		  except:
			print('Voice error')    

Room cleaning"## done
Extra towels, soaps, brush"## 		" = human help needed

add some content in what can you do, tourism info
news, sports updates
...
open bot automatically at startup
search features in hotel management website, software, apps
book travel tickets
where are you coming from
daily and hourly rooms
backup everything to a file
purpose of coming
extra remarks if any

room number_names save to file, always refresh
enable start listening
send otp

//'call human' new code
//taxi and food new code
//add log(text) to add all data to backup file
//Ask destination for taxi
//Add 'Feedback : ' when entering feedback
//change in code. 'problem' in room: 'room' to 'msg'
play short.mp3 at starting
//pyautogui method to call human

Important:
--Make Wi-Fi work in Raspberry Pi
--Voice input

Check in##     partially pending      ## = done
Check out ##
...
Wi-Fi password##
Feedback and store in file##
Hotel information##

lifting calls  - p	p=postponed
chat online - p
...
"""
"""elif 'define' in msg:   # finds meaning of a word in dictionary
	arr=msg.split(' ')
	word=arr[1]"""
...
import py_compile
py_compile.compile("/home/praneeth/Desktop/bt.py")

or

import compileall
compileall.compile_file('/home/praneeth/Desktop/bt.py')
"""
"""
book room
Deluxe
