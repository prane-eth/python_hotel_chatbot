...
with open("./avail_rooms.txt", 'r') as file:
        temp = file.readline()
available_rooms = temp.split(' ')
available_rooms.pop()
#
file = open(./rooms_names.txt", 'r')
rooms_names = {}
while True:
        temp = file.readline()
        if temp == '' or temp=='\n':
...
def call_human(text):
        ...
        sleep(13)
        pgui.hotkey('ctrl', 't')
        pgui.typewrite('https://www.messenger.com/t/vh.praneeth')
        pgui.press('enter')
        outp("Calling human.")
        sleep(20)
        pgui.click(263, 357)
...
def resp(msg):                      # for correct responses
        if 'hi' in msg or 'hello' in msg or 'hey' in msg:
                ...
                with open("./info.txt", 'r') as file11:
                        outp(file11.read())
        elif "music" in msg or 'song' in msg:
                fil_nam = random.choice(os.listdir("./songs/"))
                if vo_work:
                        outp("Playing music...")
                        mixer.music.load('./songs/'+fil_nam)
                        mixer.music.play()
                else:
                ...
                with open("./what_can_do.txt", 'r') as file0:
                        outp(file0.read())
